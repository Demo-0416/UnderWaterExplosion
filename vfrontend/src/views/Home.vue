...
<!--    <el-col :span="2" :offset="4" style="display: flex;align-items: center;font-size: 14px;">欢迎您，{{name}}</el-col>
    ...
    <el-container>
      <el-aside width="200px">
        ...
        <el-row style="">
          <el-col :span="20" :offset="2"> 
            <el-cascader style="width: 100%;" v-model="sensor_value" :options="sensor_type_options" @change="fentch_ori_data" />
          </el-col>
      ...
      <el-main>
        ...
        <el-select v-model="algorithm_value"
        placeholder="请选择算法"
...
const fentch_pre_data = async()=>{
  try {
    ...
    const response = await axios.get('http://127.0.0.1:8000/data_management/get_data',{params:pre_param});
    if(response.data.code == 0) {
...
const fentch_feature_data = async()=>{
  try {
    ...
    const response = await axios.get('http://127.0.0.1:8000/data_management/get_data',{params:feature_param});
    if(response.data.code == 0) {
      ...
      feature_chart_Opts.value = {
        ...
        series: [{
          ...
          type: 'bar',
          data: [feature_sensor_data.value.Max,feature_sensor_data.value.Min,feature_sensor_data.value.Mean,feature_sensor_data.value.StdDev,feature_sensor_data.value.PeakToPeak],
          symbol: 'none',
          itemStyle: {
            color: '#'+(Math.floor(Math.random()*16777215).toString(16)), // Random color
          },
