...
const generateData = async () => {
  try {
    const response = await axios.get('http://127.0.0.1:8000/data_management/stream_sensor_data');
...
const fetchData = async () => {
  ...
  try {
    ...
    console.log("Acceleration_data", Acceleration_data.value);
...
watch(Acceleration_data, (newVal) => {
      Acc_Charts.value = Acc_Charts.value = (() => {
 ...
 for (let sensor_data of Acceleration_data.value) {
  ...
  const chart = {
    ...
    options: {
      ...
      series: [{
        data: [[1.2,5],[2.3,6],[3.4,7],[4.5,8],[5.6,9]],
        type: 'line'
      }]
    }
  };
